...
describe("scripts/hash-password.ts", () => {
  it("prints a hash that verifies against the password it was given", async () => {
    const { stdout } = await run("pnpm", ["-s", "tsx", "scripts/hash-password.ts", "hunter2"]);
  ...
  it("exits non-zero when given no password", async () => {
    await expect(run("pnpm", ["-s", "tsx", "scripts/hash-password.ts"])).rejects.toMatchObject({
      code: 1,
    });
  }, 60_000);
});
